...
# Callback observer design

The machine owns an active `One<Operation>` and `Operation` derives from
`Pte<Operation>`. A supplied completion functor holds `Ptr<Operation>` and its
